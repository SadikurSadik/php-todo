...
<div id="app">
    ...
    <div id="todo-wrapper">
        <div class="todo-input">
            <input id="name" @keyup.enter="storeTodo($event.target.value); $event.target.value=''"
        ...
        <div v-show="showTodoDetails()" class="todo-list">
            <ul>
                <li @mouseover="mousehover(todo)" @mouseleave="mouseleave(todo)" v-for="todo in filterTodos()">
                    ...
                    <s style="color: #CCC;" v-if="todo.status == 1">{{ todo.name }}</s>
                    <span v-else v-show="todo.edit == false" @dblclick="todo.edit = true">{{ todo.name }}</span>
        ...
        <div v-show="showTodoDetails()" class="todo-footer">
            ...
            <div id="btn-group" class="f-left text-center" style="width: 198px;">
                <a v-for="tab in tabs" v-on:click="tabClick(tab)" v-bind:class="{ active: activeTab == tab.text }"
                   class="actionBtn " href="#">{{ tab.text }}</a>
            </div>
